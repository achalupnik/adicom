...
<?php foreach($all_modules_rights['module_id'] as $key_module_id => $module): ?>
    ...
    <?php foreach($module['right_id'] as $key_right_id => $right_name): ?>
        <div class="width-half">
            <input type="checkbox" id='<?=$key_module_id;?>_<?=$key_right_id;?>' class="i_ch_right" <?=(isset($group_modules_rights['module_id'][$key_module_id]['right_id'][$key_right_id])&&$group_modules_rights['module_id'][$key_module_id]['right_id'][$key_right_id]==1)?'checked':'' ?>>
            <label class="noselect" for="<?=$key_module_id;?>_<?=$key_right_id;?>"><?=$right_name;?></label>
        </div>
    <?php endforeach; ?>

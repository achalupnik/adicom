  ...
  */
  class Datatables
  {
    ...
    */
    ...
    {
      $this->not_like[] = array($key_condition, $val, $backtick_protect);
    ...
    */
    protected function get_filtering()
    {
      ...
      if($sSearch != '') {
        ...
        $sSearch_wyrazy = explode(' ', $sSearch);
        $pierwszy_wyraz=1;

        foreach($sSearch_wyrazy as $wyraz) {
          if($pierwszy_wyraz) $sWhere .= ' ( ';
          else $sWhere .= ' AND (';
          ...
          for($i = 0; $i < count($mColArray); $i++) {
            ...
            if($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)) {
              if(!$pierwsza_kolumna) $sWhere .= ' OR ';
              $sWhere .= $this->select[$mColArray[$i]] . " LIKE '%" . $wyraz . "%' ";
              $pierwsza_kolumna=0;
            }
          }
    ...
    */
    protected function get_total_results($filtering = FALSE)
    {
	  ...
	  foreach($this->select as $column)
		  $this->ci->db->select($column, false);
